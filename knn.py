...
D = KNeighborsClassifier(n_neighbors=7)

objects = ('KNN1','KNN3','KNN5', 'KNN7')
clf = [A,B,C,D]
accuracy_sc = [0,0,0,0]
...
for a in range(0,4):
    ...
    y_predict = predict_labels(clf[a], X_test)
    accuracy_sc[a] = accuracy_score(y_test, y_predict, normalize=True, sample_weight=None)
    f1_sc = f1_score(y_test, y_predict)
    print("Accuracy in %:")
    print(accuracy_sc[a]*100)
    print("\n")
    print("F1 Score:")
    print(f1_sc)
    print("\n")